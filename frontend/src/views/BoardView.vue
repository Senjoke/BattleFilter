    ...
    <div class="max-w-7xl mx-auto space-y-8">
      ...
      <section class="bg-white shadow rounded-lg overflow-hidden">
        <div class="px-4 py-5 sm:px-6 border-b border-gray-200 flex justify-between items-center">
          ...
          <div class="flex items-center space-x-2" v-if="uniqueGroups.length > 0">
            <span class="text-sm text-gray-500 font-medium">按对战组筛选:</span>
            <select v-model="selectedGroupId" class="border border-gray-300 rounded-md text-sm py-1 pl-2 pr-6 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
              <option value="all">显示全部</option>
              <option v-for="(groupId, index) in uniqueGroups" :key="groupId" :value="groupId">
                对战组 {{ index + 1 }}
              </option>
            </select>
          </div>
        ...
        <div class="p-6">
          <div v-if="filteredTeams.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <!-- 队伍卡片 -->
            <div v-for="team in filteredTeams" :key="team.id" class="bg-gray-50 border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
              <h3 class="text-lg font-semibold text-blue-600 mb-4 border-b pb-2">{{ team.name }}</h3>
              <ul class="space-y-2">
                <li v-for="player in team.players" :key="player.id" class="flex justify-between items-center text-sm">
      ...
      <section class="bg-white shadow rounded-lg overflow-hidden">
        ...
        <div class="p-6">
          <div v-if="schedules.length > 0" class="space-y-4">
            <!-- 比赛场次列表 -->
            <div v-for="match in schedules" :key="match.id" class="flex flex-col sm:flex-row items-center justify-between p-4 bg-gray-50 border border-gray-200 rounded-lg">
...
const filteredTeams = computed(() => {
  ...
  return teams.value.filter(t => t.groupId === selectedGroupId.value);
});
...
const fetchBoardData = async () => {
  try {
    ...
    const teamsRes: any = await request.get('/board/teams');
    if (teamsRes.success) {
      teams.value = teamsRes.data.map((t: any) => ({
        id: t.id,
        name: t.name,
        groupId: t.group_id,
    ...
    const matchesRes: any = await request.get('/board/matches');
    if (matchesRes.success) {
      schedules.value = matchesRes.data.map((m: any) => {
        let statusText = '未开始';
