...
router.post('/registrations', async (req, res) => {
    ...
    try {
        const checkResult = await db_1.default.query('SELECT id FROM registrations WHERE (battle_tag = $1 OR wechat_id = $2) AND tenant_id = $3', [battleTag, wechatId, req.tenantId]);
...
router.delete('/admin/registrations/clear', auth_1.authMiddleware, async (req, res) => {
    try {
        await db_1.default.query('DELETE FROM registrations WHERE tenant_id = $1', [req.tenantId]);
        redis_1.default.del(`board:registrations:${req.tenantId}`).catch(() => { });
