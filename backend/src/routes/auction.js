...
const rankOrder = new Map(rankTiers.map((tier, index) => [tier, index]));
const roleLabels = {
    tank: '重装',
    damage: '输出',
    support: '支援'
};
const jsonOk = (res, message, data) => {
    const response = { success: true, code: 200, message, data };
    return res.status(200).json(response);
};
const jsonError = (res, status, message) => {
    const response = { success: false, code: status, message, data: null };
...
const normalizeTeamCount = (value, fallback, totalPlayers) => {
...
const getRegistrationPlayer = (row) => {
...
const getRegistrations = async (tenantId) => {
    const result = await db_1.default.query('SELECT id, battle_tag, wechat_group, primary_roles, secondary_roles, self_ranks FROM registrations WHERE tenant_id = $1 ORDER BY created_at ASC', [tenantId]);
    return result.rows.map(getRegistrationPlayer);
};
const buildRankOptions = (registrations, teamCount) => {
...
const serializePlayer = (player, viewer, anonymousMode, area) => {
...
const getSession = async (tenantId) => {
    const result = await db_1.default.query('SELECT * FROM auction_sessions WHERE tenant_id = $1 LIMIT 1', [tenantId]);
    return result.rows[0] || null;
};
const buildAuctionState = async (tenantId, viewer, teamCountInput) => {
    ...
    const playersResult = await db_1.default.query(`SELECT ap.id AS "auctionPlayerId", ap.registration_id AS "registrationId", ap.status, ap.team_id AS "teamId",
    ...
    const teamsResult = await db_1.default.query(`SELECT at.*, r.battle_tag, r.wechat_group, r.primary_roles, r.secondary_roles, r.self_ranks
    ...
    if (Boolean(session.anonymous_mode) !== anonymousMode) {
        await db_1.default.query('UPDATE auction_sessions SET anonymous_mode = $1, updated_at = NOW() WHERE id = $2 AND tenant_id = $3', [anonymousMode, session.id, tenantId]);
    }
    ...
    if (currentPlayer) {
        const highestBidResult = await db_1.default.query(`SELECT b.team_id AS "teamId", b.bid_amount AS "bidAmount"
         FROM auction_bids b
        WHERE b.tenant_id = $1
          AND b.session_id = $2
          AND b.player_registration_id = $3
        ORDER BY b.bid_amount DESC, b.created_at ASC
        LIMIT 1`, [tenantId, session.id, currentPlayer.registrationId]);
        ...
        if (highestBid) {
            ...
            if (derivedTeamId !== highestTeamId || derivedBidAmount !== currentHighestBid) {
                await db_1.default.query(`UPDATE auction_sessions
              SET current_highest_team_id = $1,
                  current_highest_bid = $2,
                  updated_at = NOW()
            WHERE id = $3 AND tenant_id = $4`, [derivedTeamId, derivedBidAmount, session.id, tenantId]);
            }
...
const captainAuthMiddleware = async (req, res, next) => {
    ...
    if (!token)
        return jsonError(res, 401, '请先登录拍卖系统');
    const secret = process.env.JWT_SECRET;
    if (!secret)
        return jsonError(res, 500, '服务端 JWT_SECRET 未配置');
    try {
        const decoded = jsonwebtoken_1.default.verify(token, secret);
        if (decoded.type !== 'auction_captain')
            return jsonError(res, 401, '拍卖登录信息无效');
        const requestTenantId = req.tenantId || 'default';
        if (decoded.tenantId !== requestTenantId)
            return jsonError(res, 403, '无权访问该社区拍卖');
        const teamResult = await db_1.default.query('SELECT id FROM auction_teams WHERE id = $1 AND session_id = $2 AND tenant_id = $3', [decoded.teamId, decoded.sessionId, requestTenantId]);
        if (teamResult.rows.length === 0)
            return jsonError(res, 401, '拍卖队长身份已失效');
        req.auctionCaptain = decoded;
        next();
    }
    catch {
...
router.get('/admin/auction/state', auth_1.authMiddleware, async (req, res) => {
    try {
...
router.get('/admin/auction/captain-rank-options', auth_1.authMiddleware, async (req, res) => {
    try {
...
router.post('/admin/auction/reset', auth_1.authMiddleware, async (req, res) => {
    try {
        await db_1.default.query('DELETE FROM auction_sessions WHERE tenant_id = $1', [req.tenantId || 'default']);
...
router.post('/admin/auction/draw-captains', auth_1.authMiddleware, async (req, res) => {
    const tenantId = req.tenantId || 'default';
    const rankTier = String(req.body?.rankTier || '');
    ...
    try {
        ...
        const registrationResult = await client.query('SELECT id, battle_tag, wechat_group, primary_roles, secondary_roles, self_ranks FROM registrations WHERE tenant_id = $1 ORDER BY created_at ASC', [tenantId]);
        ...
        const sessionResult = await client.query(`INSERT INTO auction_sessions (tenant_id, status, team_count, initial_budget, current_highest_bid, last_action, created_at, updated_at)
       VALUES ($1, $2, $3, $4, 0, $5, NOW(), NOW())
       RETURNING id`, [tenantId, 'active', teamCount, initialBudget, 'draw_captains']);
        const sessionId = Number(sessionResult.rows[0].id);
        for (const player of registrations) {
            await client.query(`INSERT INTO auction_players (tenant_id, session_id, registration_id, status, created_at, updated_at)
         VALUES ($1, $2, $3, 'pool', NOW(), NOW())`, [tenantId, sessionId, player.id]);
        }
        ...
        for (let index = 0; index < captains.length; index++) {
            ...
            for (let tries = 0; tries < 5; tries++) {
                const duplicate = await client.query('SELECT id FROM auction_teams WHERE tenant_id = $1 AND captain_code = $2', [tenantId, code]);
                if (duplicate.rows.length === 0)
                    break;
                code = generateCaptainCode();
            }
            await client.query(`INSERT INTO auction_teams (tenant_id, session_id, name, captain_registration_id, captain_code, budget_total, budget_remaining, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $6, NOW(), NOW())`, [tenantId, sessionId, `队伍 ${index + 1}`, captain.id, code, initialBudget]);
            await client.query(`UPDATE auction_players
            SET status = 'captain', updated_at = NOW()
          WHERE tenant_id = $1 AND session_id = $2 AND registration_id = $3`, [tenantId, sessionId, captain.id]);
        }
...
router.post('/admin/auction/draw-player', auth_1.authMiddleware, async (req, res) => {
    const tenantId = req.tenantId || 'default';
    const client = await db_1.default.connect();
    try {
        await client.query('BEGIN');
        ...
        const poolResult = await client.query(`SELECT registration_id
         FROM auction_players
        WHERE tenant_id = $1 AND session_id = $2 AND status = 'pool'
        ORDER BY RANDOM()
        LIMIT 1`, [tenantId, session.id]);
        ...
        WHERE tenant_id = $1 AND session_id = $2 AND player_registration_id = $3`, [tenantId, session.id, nextPlayer.registration_id]);
        await client.query(`UPDATE auction_players
          SET status = 'current', team_id = NULL, sold_price = NULL, assigned_type = NULL, updated_at = NOW()
        WHERE tenant_id = $1 AND session_id = $2 AND registration_id = $3`, [tenantId, session.id, nextPlayer.registration_id]);
        await client.query(`UPDATE auction_teams SET locked_bid_amount = 0, updated_at = NOW()
        WHERE tenant_id = $1 AND session_id = $2`, [tenantId, session.id]);
        await client.query(`UPDATE auction_sessions
        ...
        WHERE tenant_id = $2 AND id = $3`, [nextPlayer.registration_id, tenantId, session.id]);
        await client.query('COMMIT');
        const state = await buildAuctionState(tenantId, 'admin');
...
router.post('/admin/auction/finish-current', auth_1.authMiddleware, async (req, res) => {
    const tenantId = req.tenantId || 'default';
    const client = await db_1.default.connect();
    try {
        await client.query('BEGIN');
        ...
        const teamResult = await client.query('SELECT * FROM auction_teams WHERE id = $1 AND tenant_id = $2 AND session_id = $3 FOR UPDATE', [session.current_highest_team_id, tenantId, session.id]);
        const team = teamResult.rows[0];
        if (!team || Number(team.budget_remaining) < bidAmount) {
            await client.query('ROLLBACK');
            return jsonError(res, 400, '最高出价队伍剩余资金不足，不能成交');
        }
        await client.query(`UPDATE auction_teams
          SET budget_remaining = budget_remaining - $1, locked_bid_amount = 0, updated_at = NOW()
        WHERE id = $2 AND tenant_id = $3 AND session_id = $4`, [bidAmount, session.current_highest_team_id, tenantId, session.id]);
        await client.query(`UPDATE auction_players
          SET status = 'sold', team_id = $1, sold_price = $2, assigned_type = 'auction', updated_at = NOW()
        WHERE tenant_id = $3 AND session_id = $4 AND registration_id = $5`, [session.current_highest_team_id, bidAmount, tenantId, session.id, session.current_player_registration_id]);
        await client.query(`UPDATE auction_sessions
        ...
        WHERE tenant_id = $1 AND id = $2`, [tenantId, session.id]);
        await client.query('COMMIT');
        const state = await buildAuctionState(tenantId, 'admin');
...
router.post('/admin/auction/pass-current', auth_1.authMiddleware, async (req, res) => {
    const tenantId = req.tenantId || 'default';
    const client = await db_1.default.connect();
    try {
        await client.query('BEGIN');
        ...
        WHERE tenant_id = $1 AND session_id = $2 AND registration_id = $3`, [tenantId, session.id, session.current_player_registration_id]);
        await client.query(`UPDATE auction_sessions
          SET current_player_registration_id = NULL, current_highest_team_id = NULL, current_highest_bid = 0,
...
router.post('/admin/auction/manual-assign', auth_1.authMiddleware, async (req, res) => {
    const tenantId = req.tenantId || 'default';
    ...
    try {
        ...
        const teamResult = await client.query('SELECT id FROM auction_teams WHERE id = $1 AND tenant_id = $2 AND session_id = $3', [teamId, tenantId, session.id]);
        if (teamResult.rows.length === 0) {
            await client.query('ROLLBACK');
            return jsonError(res, 400, '目标队伍无效');
        }
        const playerResult = await client.query(`SELECT * FROM auction_players
        WHERE tenant_id = $1 AND session_id = $2 AND registration_id = $3 FOR UPDATE`, [tenantId, session.id, registrationId]);
        ...
        WHERE tenant_id = $2 AND session_id = $3 AND registration_id = $4`, [teamId, tenantId, session.id, registrationId]);
        await client.query(`UPDATE auction_sessions SET last_action = 'manual_assigned', updated_at = NOW()
        WHERE tenant_id = $1 AND id = $2`, [tenantId, session.id]);
        await client.query('COMMIT');
        const state = await buildAuctionState(tenantId, 'admin');
...
router.post('/auction/login', async (req, res) => {
    const tenantId = req.tenantId || 'default';
    const captainCode = String(req.body?.captainCode || '').trim().toUpperCase();
    if (!captainCode)
        return jsonError(res, 400, '请输入拍卖码');
    const secret = process.env.JWT_SECRET;
    if (!secret)
        return jsonError(res, 500, '服务端 JWT_SECRET 未配置');
    try {
        const result = await db_1.default.query(`SELECT at.id AS "teamId", at.session_id AS "sessionId"
         FROM auction_teams at
         JOIN auction_sessions s ON s.id = at.session_id
        WHERE at.tenant_id = $1 AND at.captain_code = $2 AND s.status = 'active'
        LIMIT 1`, [tenantId, captainCode]);
        const team = result.rows[0];
        if (!team)
            return jsonError(res, 401, '拍卖码无效或已失效');
        const payload = {
...
router.get('/auction/state', captainAuthMiddleware, async (req, res) => {
    try {
        const captain = req.auctionCaptain;
...
router.post('/auction/bid', captainAuthMiddleware, async (req, res) => {
    const captain = req.auctionCaptain;
    ...
    try {
        await client.query('BEGIN');
        const sessionResult = await client.query('SELECT * FROM auction_sessions WHERE id = $1 AND tenant_id = $2 FOR UPDATE', [captain.sessionId, captain.tenantId]);
        const session = sessionResult.rows[0];
        if (!session?.current_player_registration_id) {
            await client.query('ROLLBACK');
            return jsonError(res, 400, '当前没有正在拍卖的队员');
        }
        const teamResult = await client.query('SELECT * FROM auction_teams WHERE id = $1 AND session_id = $2 AND tenant_id = $3 FOR UPDATE', [captain.teamId, captain.sessionId, captain.tenantId]);
        const team = teamResult.rows[0];
        if (!team) {
            await client.query('ROLLBACK');
            return jsonError(res, 401, '拍卖队长身份已失效');
        ...
        await client.query(`INSERT INTO auction_bids (tenant_id, session_id, team_id, player_registration_id, bid_amount, created_at)
       VALUES ($1, $2, $3, $4, $5, NOW())`, [captain.tenantId, captain.sessionId, captain.teamId, session.current_player_registration_id, bidAmount]);
        await client.query(`UPDATE auction_teams SET locked_bid_amount = 0, updated_at = NOW()
        WHERE tenant_id = $1 AND session_id = $2`, [captain.tenantId, captain.sessionId]);
        await client.query(`UPDATE auction_teams SET locked_bid_amount = $1, updated_at = NOW()
        WHERE tenant_id = $2 AND session_id = $3 AND id = $4`, [bidAmount, captain.tenantId, captain.sessionId, captain.teamId]);
        await client.query(`UPDATE auction_sessions
          SET current_highest_team_id = $1, current_highest_bid = $2,
              last_action = 'bid', updated_at = NOW()
        WHERE tenant_id = $3 AND id = $4`, [captain.teamId, bidAmount, captain.tenantId, captain.sessionId]);
        await client.query('COMMIT');
        const state = await buildAuctionState(captain.tenantId, 'captain');
