...
router.post('/registrations', async (req: Request, res: Response) => {
  ...
  try {
    ...
    await pool.query(
      'INSERT INTO registrations (battle_tag, wechat_id, primary_roles, secondary_roles, self_ranks, period_id, created_at) VALUES ($1, $2, $3, $4, $5, $6, NOW())',
      [battleTag, wechatId, JSON.stringify(primaryRoles), JSON.stringify(secondaryRoles || []), JSON.stringify(selfRanks || {}), periodId]
    );
...
router.get('/admin/registrations', authMiddleware, async (req: Request, res: Response) => {
  ...
  try {
    const result = await pool.query(
      'SELECT id, battle_tag as "battleTag", wechat_id as "wechatId", primary_roles as "primaryRoles", secondary_roles as "secondaryRoles", self_ranks as "selfRanks", queried_ranks as "queriedRanks", period_id as "periodId", created_at as "createdAt" FROM registrations WHERE period_id = $1 ORDER BY created_at DESC',
      [periodId]
    );
